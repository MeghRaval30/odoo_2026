...
#: The seeded roster, by the code and email the seed issues. The payslip file
#: has to name people who actually exist, because unlike an employee import a
#: payslip import matches rather than creates -- a file of strangers would
#: demonstrate nothing except the error path.
ROSTER = [
    ("EMP/2025/0001", "Aarav Mehta", "aarav@oxp.com", 85000),
    ("EMP/2025/0002", "Sara Khan", "sara@oxp.com", 95000),
    ("EMP/2025/0003", "John Dsouza", "john@oxp.com", 110000),
    ("EMP/2025/0004", "Neha Patel", "neha@oxp.com", 72000),
    ("EMP/2025/0005", "Anita Oliver", "anita@oxp.com", 78000),
    ("EMP/2025/0006", "Audrey Peterson", "audrey@oxp.com", 88000),
    ("EMP/2025/0007", "Billy Kyle", "billy@oxp.com", 105000),
    ("EMP/2025/0008", "Eli Lambert", "eli@oxp.com", 92000),
    ("EMP/2025/0009", "Paul Williams", "paul@oxp.com", 76000),
    ("EMP/2025/0010", "Priya Sharma", "priya@oxp.com", 82000),
    ("EMP/2025/0011", "Rohan Verma", "rohan@oxp.com", 68000),
    ("EMP/2025/0012", "Meera Iyer", "meera@oxp.com", 74000),
    ("EMP/2025/0013", "Karan Singh", "karan@oxp.com", 71000),
    ("EMP/2025/0014", "Divya Nair", "divya@oxp.com", 79000),
    ("EMP/2025/0015", "Arjun Reddy", "arjun@oxp.com", 118000),
    ("EMP/2025/0016", "Sneha Joshi", "sneha@oxp.com", 84000),
    ("EMP/2025/0017", "Vikram Rao", "vikram@oxp.com", 96000),
    ("EMP/2025/0018", "Ananya Gupta", "ananya@oxp.com", 89000),
    ("EMP/2025/0019", "Rahul Kapoor", "rahul@oxp.com", 81000),
    ("EMP/2025/0020", "Ishita Bose", "ishita@oxp.com", 55000),
]

#: Three months before the seeded payruns begin, so an import extends the
#: history backwards rather than colliding with it. December 2025 onward is
#: already in the system; September to November is the gap this file fills.
HISTORY_MONTHS = [("Sep-2025", 30), ("Oct-2025", 31), ("Nov-2025", 30)]


def _split(wage):
    """
    One month of pay, broken up the way an Indian payroll actually breaks it.

    Basic is 40% of gross, HRA is 40% of basic, conveyance and medical are the
    fixed statutory-ish amounts, and special allowance absorbs the remainder so
    the components sum exactly to the gross. PF is 12% of basic capped at the
    15,000 wage ceiling, PT is Maharashtra's 200 a month.
    """
    basic = round(wage * 0.40)
    hra = round(basic * 0.40)
    conveyance = 1600
    medical = 1250
    special = wage - (basic + hra + conveyance + medical)

    pf = round(min(basic, 15000) * 0.12)
    pt = 200
    tds = round(wage * 0.05) if wage >= 70000 else 0
...
def build_payslip_history(path):
    """
    A payroll register exported from the outgoing system: three months, twenty
    people, and four things wrong with it.

    The four defects are each aimed at a different check, because a file where
    everything works proves only that the happy path runs:

      * **Vikram Rao's October row is short one allowance.** Every component is
        present and plausible; the special allowance cell is simply blank while
        the stated net still includes it. Nothing about the row looks wrong --
        which is exactly why the arithmetic check exists. It comes out 39,390
        under the gross and the net the sheet itself states.
      * **A row for someone who does not work here.** 'Mohit Bhatia' was on the
        old system and never migrated. It cannot resolve to a person, so it is
        refused rather than guessed at.
      * **Sara Khan appears twice in November** -- once under her code and once
        under her name, as happens when a register is assembled from two
        branches. The second is caught against the first.
      * **A December row**, already paid in this system. It trips the same
        duplicate guard the payrun wizard uses, from the other direction.

    Structurally the file is what a legacy export looks like: a title row above
    the header, abbreviated headers, months as 'Sep-2025', and amounts written
    with thousands separators as text.
    """
    header = ["Emp Code", "Employee Name", "Month", "Paid Days", "LOP",
              "Basic", "HRA", "Conveyance", "Medical", "Spl Allow",
              "Gross Salary", "PF", "P.Tax", "TDS", "Total Ded", "Net Pay"]

    rows = [
        ["PAYROLL REGISTER - SEPTEMBER TO NOVEMBER 2025", "", "", "", "", "",
         "", "", "", "", "", "", "", "", "", ""],
        header,
    ]

    def emit(code, name, month, days, lop, parts, drop_special=False,
             keep_stated_net=False):
        special = "" if drop_special else parts["special"]
        earnings = (parts["basic"] + parts["hra"] + parts["conveyance"]
                    + parts["medical"] + parts["special"])
        deductions = parts["pf"] + parts["pt"] + parts["tds"]
        # The stated totals are what the old system printed. When a component
        # is dropped they are left untouched on purpose -- that is the whole
        # defect, and repairing them here would repair the demo away.
        gross = earnings
        net = earnings - deductions
        return [
            code, name, month, days, lop,
            parts["basic"], parts["hra"], parts["conveyance"],
            parts["medical"], special,
            "{:,}".format(gross), parts["pf"], parts["pt"], parts["tds"],
            deductions, "{:,}".format(net),
        ]

    for month, days_in_month in HISTORY_MONTHS:
        for code, name, _email, wage in ROSTER:
            parts = _split(wage)
            lop = 0
            paid = days_in_month

            # One person takes two days of unpaid leave in October, so the
            # attendance columns are not uniformly the same number.
            if code == "EMP/2025/0011" and month == "Oct-2025":
                lop, paid = 2, days_in_month - 2

            drop = (code == "EMP/2025/0017" and month == "Oct-2025")
            rows.append(emit(code, name, month, paid, lop, parts,
                             drop_special=drop))

    # -- the four defects ---------------------------------------------------

    # Somebody the old system paid who was never migrated here.
    stranger = _split(64000)
    rows.append(_emit_plain("EMP/2025/0099", "Mohit Bhatia", "Oct-2025", 31,
                            stranger))

    # The same person twice in one month, from two branch registers.
    duplicate = _split(95000)
    rows.append(_emit_plain("", "Sara Khan", "Nov-2025", 30, duplicate))

    # A month this system has already paid.
    already = _split(85000)
    rows.append(_emit_plain("EMP/2025/0001", "Aarav Mehta", "Dec-2025", 31,
                            already))

    _write_sheet(path, "Salary Register", rows, header_row=2,
                 widths=[15, 20, 11, 10, 7, 11, 11, 12, 10, 11, 13, 9, 8, 9,
                         11, 12])
    return len(rows) - 2


def _emit_plain(code, name, month, days, parts):
    earnings = (parts["basic"] + parts["hra"] + parts["conveyance"]
                + parts["medical"] + parts["special"])
    deductions = parts["pf"] + parts["pt"] + parts["tds"]
    return [
        code, name, month, days, 0,
        parts["basic"], parts["hra"], parts["conveyance"], parts["medical"],
        parts["special"], "{:,}".format(earnings), parts["pf"], parts["pt"],
        parts["tds"], deductions, "{:,}".format(earnings - deductions),
    ]
...
FILES = [
    ...
    ("07-legacy-payslip-history.xlsx", build_payslip_history),
]
